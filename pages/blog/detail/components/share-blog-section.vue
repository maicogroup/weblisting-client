<template>
	<div class="flex flex-col items-center space-y-10 w-fit">
		<p class="text-sm whitespace-nowrap text-stone-900">Chia sẻ</p>
		<button>
			<a
			...
			>
				<img
					class="h-10 w-10"
					src="~/static/images/share-buttons/facebook-logo.png"
				/>
			</a>
		</button>
	...
	methods: {
		configZaloPanel() {
			...
			const handleZaloPanelPosition = () => {
				...
				zaloPanel.style.top =
					zaloButtonRect.top - zaloPanelRect.height + 25 + "px";
				zaloPanel.style.left =
					zaloButtonRect.left - zaloPanelRect.width - 10 + "px";
			};

			window.addEventListener("scroll", handleZaloPanelPosition);
			const observer = new MutationObserver(mutations =>
				mutations.forEach(handleZaloPanelPosition),
			);

			// thay đổi vị trí của panel mỗi style bị thay đổi
			observer.observe(zaloPanel, {
				attributes: true,
