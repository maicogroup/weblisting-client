<template>
	<div v-if="project" class="w-full sm:px-36 px-5 color-stone-900">
		<div class="flex flex-col-reverse md:flex-col">
			<div>
				<div
					class="text-sm font-medium sm:font-normal color-999999 mt-2.5 sm:mt-0 mb-1.5 sm:mb-2.5"
				>
					<a href="#">Dự án</a>
					/
					<a href="">TP HCM</a>
					/
					<a class="font-bold color-stone-900" href="#">{{
						project.projectName
					}}</a>
				</div>
				<h1
					class="font-bold font-size-project-name mb-1 mt-2 color-stone-900"
					@click="HandleScroll()"
				>
					{{ project.projectName }}
				</h1>
		...
		<div class="mt-1 sm:mt-5 sm:mx-44">
			<div
				class="flex justify-between sm:justify-start sticky-table-of-content z-10 sm:pt-0 pt-3 color-stone-900"
			>
				<button
					:class="
						isTargetingTableOfContents.isTargetingOverview
							? activeTableContent
							: tableContentStyles
					"
				...
				<button
					:class="
						isTargetingTableOfContents.isTargetingGround
							? activeTableContent
							: tableContentStyles
					"
				...
				<button
					:class="
						isTargetingTableOfContents.isTargetingLocation
							? activeTableContent
							: tableContentStyles
					"
				...
				<button
					:class="
						isTargetingTableOfContents.isTargetingRentAndSell
							? activeTableContent
							: tableContentStyles
					"
			...
			<div
				ref="Overview"
				class="color-stone-900"
				style="scroll-margin-top: 135px"
				v-on:="ScrollToOverviewHandle()"
			>
				...
				<div class="grid sm:grid-cols-2 grid-cols-1 sm:gap-6 h-auto">
					...
					<div class="block">
						<p class="re-properties-frame">
							...
							<span class="re-properties-content max-w-40">
								{{ project.numberOfApartments }} căn
							</span>
						</p>
						<p class="re-properties-frame">
							<span class="re-properties flex w-7/12">Số tòa</span>
							<span class="re-properties-content max-w-40">
								{{ project.numberOfBuildings }}</span
							>
						</p>
						<p class="re-properties-frame">
							<span class="re-properties flex w-7/12">Pháp lý</span>
							<span class="re-properties-content max-w-40">
								{{ project.juridical }}</span
							>
						</p>
					</div>
				</div>
				<div
					:class="`${showReadMoreStyles} ql-editor`"
					style="overflow: hidden"
					v-html="project.projectPost"
				/>
			</div>
			<button
				class="flex items-center ml-auto mr-auto mt-6 mb-8"
				@click="setReadMoreActiveState()"
			>
			...
			<expand-panel title="Thông tin chi tiết" class="mt-2.5 sm:mt-9">
				<div
					class="grid md:grid-cols-2 grid-cols-1 md:gap-6 h-auto"
					style="column-gap: 20px"
				>
					<p
						v-for="(item, index) in detailInfors"
						:key="index"
						class="re-properties-frame-panel"
					>
						<span class="re-properties-panel flex w-1/2">{{ item.name }}</span>
						<span class="re-properties-content max-w-50">{{
							item.content
						}}</span>
					</p>
				</div>
			...
			<expand-panel title="Tiện ích nội khu" class="sm:mt-9 mt-3.5">
				<div>
					<ul
						class="list-none grid grid-cols-2 gap-2 text-sm font-medium color-stone-900"
					>
						<li
							v-for="utility in project.utilities.listOfUtilities"
							:key="utility"
							class="mb-2.5"
						>
							<p>{{ utility }}</p>
					...
					<div class="sm:ml-12 grow">
						...
						<div class="">
							<div class="text-lg flex justify-start mt-3">
								...
								<P class="max-w-60">
									{{ project.investor.numberOfProjects }} sắp mở -
									{{ project.investor.numberOfProjects }} đang mở bán -
			...
			<h2
				ref="Ground"
				class="font-size-project-ground mt-12 sm:mt-6 mb-1.5 sm:mb-4"
				style="scroll-margin-top: 135px"
			>
				Mặt bằng dự án
			</h2>
...
export default {
	...
	data() {
		return {
			...
			tableContentStyles:
				"text-center sm:mr-6 table-of-content color-A3A3A3 transition-02s-all",
			activeTableContent:
				"active-table-contents text-center sm:mr-6 transition-02s-all",
	...
	head() {
		return {
			title: this.project?.pageInfors.find(c =>
				c.slug.includes(this.$route.params.slug),
			).title,
			meta: [
				{
					hid: "description",
	...
	computed: {
		...
		detailInfors() {
			const items = [
				{ name: "Giá", content: this.project.price + " triệu/m²" },
				{ name: "Diện tích", content: this.project.acreage + " m²" },
				{ name: "Chủ đầu tư", content: this.project.investor.investorName },
				{ name: "Số căn hộ", content: this.project.numberOfApartments },
