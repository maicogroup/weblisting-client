<template>
  <div class="w-72 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-8 bg-white rounded-md shadow-xl">
    ...
    <button class="block mt-2 mx-auto text-sm text-blue-500 hover:text-blue-600" @click="$emit('signup')">
...
export default {
  apollo: {
    user: {
      query: gql`query GetGuestUser($phoneNumber: String!) {
  ...
  methods: {
    ...
    signIn () {
      ...
      this.$emit('success', { name: this.user.name, phoneNumber: this.user.phoneNumber });
      this.loginName = '';
    }
  }
};
