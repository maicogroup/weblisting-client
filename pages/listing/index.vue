<template>
	<div class="w-full max-w-screen-xl px-4">
		...
		<div class="relative my-4">
			<div class="w-full block md:hidden" @click="openCloseMobileFilter">
				<span class="absolute inset-y-0 left-0 flex items-center pl-2">
					...
					<h4 class="focus:text-gray-900 focus:outline-none text-gray-400">
						Tìm kiếm
					</h4>
				</span>
				<input
					type="button"
					class="w-full border py-2 pl-10 pr-2 h-full rounded-md"
					placeholder="Tìm kiếm..."
				/>
			</div>
		</div>
		...
		<div class="flex justify-between w-full">
			<ListPost
				v-if="waitTillProjectIsDetermined"
				class="left-0"
				:filter="filter"
				:heading="heading"
			/>
			<div class="ml-9 mt-14 hidden lg:block">
				...
				<div class="border mt-4 p-4 rounded">
					<p class="font-bold mb-2">Xem theo giá</p>
					<div v-if="filter.demand === 'Cho Thuê'">
						<quick-filter-button
							@click="
								updateUrlQueryAndNavigate({ priceFrom: undefined, priceTo: 3 })
							"
						>
							Dưới 3 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 3, priceTo: 5 })"
						>
							Từ 3 - 5 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 5, priceTo: 7 })"
						>
							Từ 5 - 7 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 7, priceTo: 10 })"
						>
							Từ 7 - 10 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 10, priceTo: 15 })"
						>
							Từ 10 - 15 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 15, priceTo: 20 })"
						>
							Từ 15 - 20 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 20, priceTo: 30 })"
						>
							Từ 20 - 30 triệu
						</quick-filter-button>
						<quick-filter-button
							@click="
								updateUrlQueryAndNavigate({ priceFrom: 30, priceTo: undefined })
							"
						>
							Trên 30 triệu
						</quick-filter-button>
					...
					<div v-else>
						<quick-filter-button
							@click="
								updateUrlQueryAndNavigate({ priceFrom: undefined, priceTo: 1 })
							"
						>
							Dưới 1 tỷ
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 1, priceTo: 2 })"
						>
							Từ 1 tỷ - đến 2 tỷ
						</quick-filter-button>
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 2, priceTo: 3 })"
						>
							Từ 2 tỷ - đến 3 tỷ
						</quick-filter-button>
						<quick-filter-button
						...
						<quick-filter-button
							@click="updateUrlQueryAndNavigate({ priceFrom: 10, priceTo: 20 })"
						>
							Từ 10 tỷ - đến 20 tỷ
						</quick-filter-button>
						<quick-filter-button
							@click="
								updateUrlQueryAndNavigate({ priceFrom: 20, priceTo: undefined })
							"
						>
							Trên 20 tỷ
						</quick-filter-button>
				...
				<div class="border mt-4 p-4 rounded">
					<p class="font-bold mb-2">Xem theo diện tích</p>
					<quick-filter-button
						@click="
							updateUrlQueryAndNavigate({
								acreageFrom: undefined,
								acreageTo: 30,
							})
						"
					>
					...
					<quick-filter-button
						@click="
							updateUrlQueryAndNavigate({ acreageFrom: 30, acreageTo: 50 })
						"
					>
						Từ 30 - 50 m²
					</quick-filter-button>
					<quick-filter-button
						@click="
							updateUrlQueryAndNavigate({ acreageFrom: 50, acreageTo: 70 })
						"
					>
						Từ 50 - 70 m²
					</quick-filter-button>
					<quick-filter-button
						@click="
							updateUrlQueryAndNavigate({ acreageFrom: 70, acreageTo: 100 })
						"
					>
						Từ 70 - 100 m²
					</quick-filter-button>
				...
				<div class="border mt-4 p-4 rounded">
					<p class="font-bold mb-2">Xem theo phòng ngủ</p>
					<quick-filter-button
						@click="updateUrlQueryAndNavigate({ bedroomOptions: ['1'] })"
					>
						1 phòng ngủ
					</quick-filter-button>
					<quick-filter-button
						@click="updateUrlQueryAndNavigate({ bedroomOptions: ['2'] })"
					>
						2 phòng ngủ
					</quick-filter-button>
					<quick-filter-button
						@click="updateUrlQueryAndNavigate({ bedroomOptions: ['3'] })"
					>
						3 phòng ngủ
					</quick-filter-button>
					<quick-filter-button
						@click="updateUrlQueryAndNavigate({ bedroomOptions: ['4'] })"
					>
						4 phòng ngủ
					</quick-filter-button>
					<quick-filter-button
						@click="updateUrlQueryAndNavigate({ bedroomOptions: ['5+'] })"
					>
						5+ phòng ngủ
					</quick-filter-button>
				</div>
		...
		<Divider
			v-if="showIfPostsOfOneProject"
			class="border-stone-400 mt-7 mb-8 sm:mb-12"
		/>
		<ProjectHeader
			v-if="showIfPostsOfOneProject"
			class="mb-5"
		...
		/>
		<div
			v-if="project && showIfPostsOfOneProject && project.seoContent"
			class="rounded-lg border mr-auto md:w-4/5 ml-auto mt-9 mb-5 md:px-6 h-fit delay-3000"
		>
			<div
			...
			/>
			<p
				class="text-center font-bold read-more my-4"
				@click="setSEOContentActiveState()"
			>
				{{ readMoreContent }}
			</p>
		</div>
		...
		>
			<div class="flex justify-between mt-5 items-center mb-4">
				<h4 class="mr-auto ml-auto text-base">Tìm kiếm bất động sản</h4>
			...
			<div class="relative mx-2.5">
				...
				<input
					type="search"
					class="w-full border py-2 pl-10 pr-2 h-11 rounded-md bg-white focus:text-gray-900 focus:outline-none"
					placeholder="Tìm kiếm..."
				/>
			</div>
			<div class="double-button my-4 flex mx-2.5">
				<button
					:class="`w-1/2 grow text-base py-2 px-4 border rounded-l-md ${sellButtonClasses}`"
					@click="setFilterDemandOption('Bán')"
				>
					Bán
				</button>
				<button
					:class="`w-1/2 grow text-base whitespace-nowrap py-2 px-4 border rounded-r-md ${rentButtonClasses}`"
					@click="setFilterDemandOption('Cho Thuê')"
				>
					Cho thuê
				</button>
			...
			<div class="mx-2.5 flex space-x-2">
				<button
					class="px-6 py-2 shrink-0 border rounded-md my-4 hover:bg-gray-100"
					@click="resetFilter"
				>
					Đặt lại
				</button>
				<button
					class="grow whitespace-nowrap rounded-md px-6 py-2 bg-green-500 hover:bg-green-700 text-white my-4"
					@click="handleFilterButtonPressed"
				>
					Tìm kiếm
				</button>
...
export default {
	...
	head() {
		...
		if (this.$route.params.pathMatch != undefined) {
			title = this.project?.pageInfors.find(c =>
				c.slug.includes(this.$route.params.pathMatch.replace("/", "")),
			).title;
		} else {
			...
			meta: [
				{
					hid: "description",
					name: "description",
					content: this.project?.pageInfors.find(c =>
						c.slug.includes(this.$route.params.pathMatch.replace("/", "")),
					).metaDescription,
				},
				{
	...
	computed: {
		...
		sellButtonClasses() {
			return this.inputFilter.demand === "Bán"
				? "bg-gray-200"
				: "hover:bg-gray-100";
		},
		...
		sEOContentClasses() {
			return this.readMoreButtonIsActive
				? "h-fit opacity-100 ql-editor"
				: "h-60 opacity-60";
		},
	},
	...
	created() {
		...
		if (this.$route.params.pathMatch)
			this.filter.demand = this.$route.params.pathMatch
				.replace("/", "")
				.includes("ban")
				? "Bán"
				: "Cho Thuê";
		else this.filter.demand = "Cho Thuê";

		this.$watch(
			() => this.$route.params,
			(param, prevParam) => {
				if (
					param.pathMatch !== prevParam.pathMatch &&
					param.pathMatch !== null
				) {
					this.$apollo.queries.project.refetch({
						slug: param.pathMatch.replace("/", ""),
					});
				}
	...
	apollo: {
		project: {
			...
			update(data) {
				...
				project.seoContent = project.forSellSEOContent
					? project.forSellSEOContent
					: project.forRentSEOContent;
	...
	methods: {
		...
		handleFilterButtonPressed() {
			...
			if (this.filter.priceRange) {
				const { from, to } = this.swapRangeValueIfInvalid(
