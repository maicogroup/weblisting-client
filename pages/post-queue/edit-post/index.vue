<template>
	<div class="px-5 w-full max-w-screen-xl mx-5">
		...
		<h2 class="font-bold text-lg mb-2">Thông tin trang</h2>
		<div class="flex flex-col w-full ml-2">
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Slug:</p>
				<br />
				<div class="flex justify-between items-center">
					<input
						:placeholder="post.pageInfor.slug"
						type="text"
						class="w-11/12 border rounded-md mb-2 pl-4"
						v-model="post.pageInfor.slug"
					/>
				</div>
			</div>
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Title:</p>

				<br />
				<div class="flex justify-between items-center">
					<input
						:placeholder="post.pageInfor.title"
						type="text"
						class="w-11/12 border rounded-md mb-2 pl-4"
						v-model="post.pageInfor.title"
					/>
				</div>
			</div>
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Meta:</p>
				<br />
				<div class="flex justify-between items-center">
					<input
						:placeholder="post.pageInfor.metaDescription"
						type="text"
						class="w-11/12 border rounded-md mb-2 pl-4"
						v-model="post.pageInfor.metaDescription"
					/>
				</div>
			</div>
		</div>
		...
		<div class="flex flex-col w-full ml-2">
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Mã căn:</p>
				<input
					:placeholder="post.apartmentNumber"
					type="text"
					class="w-full border rounded-md mb-2 pl-4"
					v-model="post.apartmentNumber"
				/>
			</div>
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Diện tích (m<sup>2</sup>):</p>
				<input
			...
			<div class="my-2">
				...
				<input
					:placeholder="post.usageAcreage"
					type="number"
					class="w-full border rounded-md mb-2 pl-4"
					v-model="post.usageAcreage"
				/>
			</div>
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Số tầng:</p>
				<input
					:placeholder="post.floor"
					type="number"
					class="w-full border rounded-md mb-2 pl-4"
					v-model="post.floor"
				/>
			...
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Block:</p>
				<input
					:placeholder="post.block"
					type="text"
					class="w-full border rounded-md mb-2 pl-4"
					v-model="post.block"
				/>
			...
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Hướng nhà:</p>
				<input
			...
			<div class="my-2">
				<p class="font-medium mr-6 text-md">
					Hướng cửa (view):
					<input
						:placeholder="post.doorDirection"
						type="text"
						class="w-full border rounded-md mb-2 pl-4"
						v-model="post.doorDirection"
					/>
				</p>
			</div>
			<div class="my-2">
				<p class="font-medium mr-6 text-md">Đặc điểm khác:</p>
				<input
					:placeholder="post.otherSpecification"
					type="text"
					class="w-full border rounded-md mb-2 pl-4"
					v-model="post.otherSpecification"
				/>
			</div>
			<div class="my-2">
				<p class="font-medium mr-6 text-md">View:</p>
				<input
					:placeholder="post.view"
					type="text"
					class="w-full border rounded-md mb-2 pl-4"
					v-model="post.view"
				/>
		...
		<div class="mb-2 mt-8 text-lg flex justify-between">
			<h2 class="font-bold">Nội thất</h2>
			<div class="flex justify-between items-center">
				<input
		...
		<div class="mt-4 rounded-lg border-8 py-3 px-5">
			<ul>
				<li
					v-for="item in post.furnitures"
					:key="item"
					class="bgc-cornflowerblue rounded-lg inline-flex mr-4 mt-2 mb-3 p-2 list-none"
					title="xóa nội thất"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
		...
		<h2 class="font-bold text-lg mb-2 mt-6">Mô tả</h2>
		<client-only>
			<quill-editor
				ref="myQuillEditor"
		...
		<h1 class="font-bold text-lg my-2">Hình ảnh</h1>
		...
		<div class="pannel mt-10" style="width: 100%">
			<div class="list row">
				<drag-sortable
					...
					:key="item"
					:index="index"
					@sortend="sortend($event, post.gallery)"
		...
		<div class="flex justify-end mt-5 space-x-2">
			<button
				@click="updatePost"
				class="bg-blue-400 hover:bg-blue-600 text-white font-bold h-10 py-2 px-4 rounded flex item-center"
			>
				Cập nhật
			</button>
			<button
				@click="publishPost"
				class="bg-green-500 hover:bg-green-700 text-white font-bold h-10 py-2 px-4 rounded flex items-center"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="mr-1 h-5 w-5"
		...
		<modal name="update-before-publish">
			...
			<div class="flex justify-center space-x-5 mt-5">
				...
				<button
					@click="updatePost"
					class="bg-green-400 hover:bg-green-600 text-white font-bold h-10 py-2 px-4 rounded flex item-center"
				>
					Cập nhật
				</button>
			</div>
...
const updatePost = gql`
	mutation deletePostTag($input: UpdatePostInput!) {
...
export default {
	...
	methods: {
		...
		addNewFurniture() {
			if (
				this.newFurniture == "" ||
				this.post.furnitures.includes(this.newFurniture)
			) {
		...
		updatePost() {
			...
			if (
				this.currentPost.pageInfor.title == this.post.pageInfor.title &&
				this.currentPost.pageInfor.slug == this.post.pageInfor.slug &&
				this.currentPost.pageInfor.metaDescription ==
					this.post.pageInfor.metaDescription &&
				this.currentPost.apartmentNumber == this.post.apartmentNumber &&
				this.currentPost.acreage == this.post.acreage &&
				this.currentPost.floor == this.post.floor &&
				this.currentPost.block == this.post.block &&
				this.currentPost.description == this.post.description &&
				this.currentPost.tags.length == this.post.tags.length &&
				this.currentPost.otherSpecification == this.post.otherSpecification &&
				this.currentPost.usageAcreage == this.post.usageAcreage &&
				this.currentPost.view == this.post.view &&
				this.currentPost.doorDirection == this.post.doorDirection &&
				this.currentPost.furnitures.length == this.post.furnitures.length &&
			...
			if (
				this.post.acreage == null ||
				this.post.acreage <= 0 ||
				this.post.floor == null ||
				this.post.floor <= 0 ||
				this.post.usageAcreage <= 0
			) {
				this.$toast.show(
					"Diện tích, Diện tích sử dụng và Tầng phải lớn hơn 0!",
					{
						type: "error",
						theme: "bubble",
						duration: 3000,
			...
			this.$apollo
				...
				.then(data => {
					...
					this.imageIsChanged = false;
					this.post.tags.forEach(tag => this.currentPost.tags.push(tag));
					this.post.furnitures.forEach(c =>
						this.currentPost.furnitures.push(c),
					);
		...
		publishPost() {
			...
			if (
				...
				this.currentPost.description != this.post.description ||
				this.currentPost.tags.length != this.post.tags.length ||
				this.currentPost.gallery.forEach(x =>
					this.post.gallery.forEach(c => x != c),
				) ||
				this.currentPost.furnitures.length != this.post.furnitures.length ||
				this.currentPost.otherSpecification != this.post.otherSpecification ||
				this.currentPost.usageAcreage != this.post.usageAcreage ||
			...
			if (
				this.post.acreage == null ||
				this.post.acreage <= 0 ||
				this.post.apartmentNumber == null ||
				this.post.apartmentNumber == "" ||
				this.post.block == null ||
				this.post.block == "" ||
				this.post.floor == null ||
				this.post.floor <= 0 ||
				this.post.pageInfor.title == null ||
				this.post.pageInfor.title == "" ||
				this.post.pageInfor.slug == null ||
				this.post.pageInfor.slug == "" ||
				this.post.pageInfor.metaDescription == null ||
				this.post.pageInfor.metaDescription == "" ||
				this.post.gallery.length == 0 ||
				this.post.furnitures.length == 0 ||
				this.post.usageAcreage == null ||
				this.post.usageAcreage <= 0 ||
				this.post.otherSpecification == null ||
				this.post.otherSpecification == "" ||
				this.post.view == null ||
				this.post.view == "" ||
				this.post.doorDirection == null ||
				this.post.doorDirection == ""
			) {
				this.$toast.show(
					"Tất cả dữ liệu không được để trống, vui lòng cập nhật và kiểm tra lại!",
					{
						type: "error",
						theme: "bubble",
						duration: 3000,
			...
			this.$apollo.mutate({
				mutation: gql`
					mutation PublishPost($input: UpdatePostStatusInput!) {
