  ...
  <div>
    ...
    >
      <div class="flex justify-between border-b rounded">
        ...
        <p class="font-semibold px-6 py-2 mr-auto ml-auto">Chọn diện tích</p>
      </div>

      <div
        class="flex justify-between h-11 border-b items-center mx-3"
        @click="handleSelectAllSize"
      >
        <p class="text-dark-red">Tất cả diện tích</p>
      ...
      >
        <div class="flex justify-start">
          <template v-if="range.from === 0"> Dưới {{ range.to }} m² </template>
          <template v-else-if="range.to === null">
            Trên {{ range.from }} m²
          </template>
          <template v-else>
            <span class="range-text"> Từ {{ range.from }} m² </span>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4 mx-2"
              fill="none"
            ...
            <span class="range-text text-right"> đến {{ range.to }} m² </span>
          </template>
        </div>
        <input
          type="radio"
...
export default {
  ...
  methods: {
    ...
    isSelected(range) {
      return (
        range?.from === this.selectedOption?.from &&
        range?.to === this.selectedOption?.to
      );
    },
