	...
	<div>
		...
		>
			<div class="flex justify-between border-b rounded">
				...
				<p class="font-semibold px-6 py-2 mr-auto ml-auto">Chọn khu vực</p>
			</div>
			...
			>
				<p class="text-dark-red">Tất cả khu vực</p>
			...
			>
				<div class="flex justify-between h-11 border-b items-center mx-3">
					...
					<div @click="isShowDistrict = false">
						<svg
							xmlns="http://www.w3.org/2000/svg"
...
export default {
	...
	computed: {
		...
		filteredDistricts() {
			const query = this.searchInput.trim().toLowerCase();
			return this.selectedCity.districts.filter(d =>
	...
	methods: {
		...
		handleSelectSecondOption(option) {
			...
			this.$emit("optionchanged", {
				city: this.firstOption,
				district: this.secondOption,
			});
