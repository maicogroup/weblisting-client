<template>
	...
	>
		...
		<div class="double-button my-4 flex mx-2.5">
			<button
				:class="`relative w-1/2 grow text-base py-2 px-4 border rounded-l-md`"
				@click="setFilterDemandOption('Bán')"
			>
				<div
					:class="`absolute top-0 w-full h-full z-10 ${sellButtonClasses}`"
				></div>
				<div :class="`text-black z-20`">Bán</div>
			</button>
			<button
				:class="`relative w-1/2 grow text-base whitespace-nowrap py-2 px-4 border rounded-r-md`"
				@click="setFilterDemandOption('Cho Thuê')"
			>
				<div
					:class="`absolute top-0 w-full h-full z-10 ${rentButtonClasses}`"
				></div>
...
export default {
	...
	computed: {
		sellButtonClasses() {
			...
			return this.filter.demand === "Bán"
				? "toogle-animation-up right-0"
				: "toogle-animation-down right-0";
		},

		rentButtonClasses() {
			if (this.filter.demand == null) return "toogle-animation-up left-0";
			return this.filter.demand !== "Bán"
				? "toogle-animation-up left-0"
				: "toogle-animation-down left-0";
		},
	},
