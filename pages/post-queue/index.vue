<template>
	<div class="mx-2" id="post-queue">
		<div class="flex justify-between space-x-3 items-center mb-5">
			<p class="text-gray-600">Hiện có {{ totalCount }} bài viết chờ duyệt</p>
		...
		<div v-if="postsWithPagination != null" class="flex-col space-y-6 mb-4">
			<div
				v-for="post in postsWithPagination.items"
				:key="post.id"
				class="bg-white rounded-md max-w-4xl mx-auto p-4 hover:shadow-lg border-gray-400 border border-solid"
			>
				<div class="flex justify-between">
					<div>
						...
						<p class="mb-2 font-semibold">
							Ngày sửa:
							<span class="font-thin">{{
								new Date(post.lastUpdatedAt).toLocaleString()
							}}</span>
						</p>
					</div>
					<div class="flex justify-end space-x-2">
						<a
							:href="`chinh-sua-tin-dang/${post.pageInfor.slug}`"
							class="bg-blue-500 hover:bg-blue-700 text-white font-bold h-10 py-2 px-4 rounded flex items-center"
						>
							<svg
				...
				<p class="truncate">
					...
					<span v-else class="mt-2 text-red-600 font-semibold"> trống </span>
				</p>
				<div>
					<p class="mt-1 font-semibold">
						Mô tả:
						<span
							v-if="!isStringNullOrWhitespace(post.description)"
							class="font-thin mt-2"
						>
							{{ formatDescription(post.description) }}
						</span>
						<span v-else class="mt-2 text-red-600"> trống </span>
					</p>
				</div>
				...
				<div class="text-purple-800">
					<p class="font-semibold">Thông tin SEO:</p>

					<p class="ml-2 font-semibold">
						Slug:
						<span
							v-if="!isStringNullOrWhitespace(post.pageInfor.slug)"
							class="font-thin mt-2"
						>
							{{ post.pageInfor.slug }}
						</span>
					...
					<p class="ml-2 mt-1 font-semibold">
						Title:
						<span
							v-if="!isStringNullOrWhitespace(post.pageInfor.title)"
							class="font-thin mt-2"
						>
					...
					<p class="ml-2 mt-1 font-semibold">
						Meta description:
						<span
							v-if="!isStringNullOrWhitespace(post.pageInfor.metaDescription)"
							class="font-thin mt-2"
						>
		...
		postsWithPagination: {
			...
			variables() {
				return {
					...
					skip: 0,
					condition: {
						or: [{ status: { eq: "Draft" } }, { status: { eq: "Updated" } }],
					},
				};
			},
		},
		...
		postPush: function (post) {
			...
			this.$apollo.mutate({
				...
				update: (store, { data: { postPush } }) => {
					...
					debugger;
					(postsWithPagination.items = postsWithPagination.items.filter(
						c => c.id != post.id,
					)),
					...
					store.writeQuery({
						...query,
						data: { postsWithPagination: postsWithPagination },
					});
				},
			});
		},
