	...
	<div class="vue-expand-panel p-2 h-auto">
		...
		<div class="flex items-center justify-center">
			<project-filter-dropdown
				:selected-option="inputFilter.project"
			...
			<div class="p-2 h-auto" style="width: 1000px">
				<expand-panel title="Thông tin">
					<div>
						<p class="mb-2">
							...
							<input
								...
								type="text"
								class="w-1/2"
								@change="() => (flags.information = true)"
							/>
						</p>
						<p class="mb-2">
							<label class="font-semibold">Pháp lý:</label>
							<input
								v-model="project.juridical"
								type="text"
								class="w-1/2"
								@change="() => (flags.information = true)"
							/>
						...
						<div class="items-start flex">
							...
							<button
								name="edit-button"
								@click="() => $modal.show('edit-project-master-plan')"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									width="17"
					...
					<div class="flex justify-end my-2">
						<button
							class="text-white px-3 py-1 bg-green-400 rounded"
							@click="updateProjectInformation"
						>
							Cập nhật
						</button>
					</div>
				...
				<expand-panel title="Thông tin chi tiết">
					...
					<div class="grid grid-cols-4">
						<div>
							...
							<input
								...
								class="w-auto"
								type="number"
								@change="() => (flags.detailInformation = true)"
							/>
						</div>
						<div>
							<p class="font-semibold">Số căn hộ</p>
							<input
								v-model="project.numberOfApartments"
								class="w-auto"
								type="number"
								@change="() => (flags.detailInformation = true)"
							/>
						...
						<div>
							<p class="font-semibold">Số tòa</p>
							<input
								v-model="project.numberOfBuildings"
								class="w-auto"
								type="number"
								@change="() => (flags.detailInformation = true)"
							/>
						...
						<div
							v-for="(item, index) in project.tempProjectInfors"
							:key="index"
						>
					...
					<div class="flex justify-end my-2">
						<button
							class="text-white px-3 py-1 bg-green-400 rounded"
							@click="updateProjectDetailInformation"
						>
							Cập nhật
						</button>
					</div>
				...
				<expand-panel title="Địa chỉ">
					<div>
						...
						<p class="mb-2">
							...
							<input
								v-model="project.address.district"
								type="text"
								class="w-1/2"
								@change="() => (flags.address = true)"
							/>
					...
					<div>
						...
						<div
							v-if="
								project.address.googleMapLocation !== null &&
								project.address.googleMapLocation.includes('https')
							"
						>
							<iframe
						...
						<div v-else>
							<em> chưa có hoặc sai! </em>
						</div>
					</div>
					<div class="flex justify-end my-2">
						<button
							class="text-white px-3 py-1 bg-green-400 rounded"
							@click="updateProjectAddress"
						>
							Cập nhật
						</button>
					</div>
				...
				<expand-panel title="Tiện ích">
					<p>
						...
						<textarea
							v-model="project.utilities.locationUtilities"
							type="text"
							class="w-1/2"
							rows="4"
							@change="() => (flags.utilities = true)"
						/>
					</p>
					<p>
						<label for="meta" class="font-semibold">Tiện ích nội khu:</label>
						<textarea
							v-model="project.utilities.internalUtilities"
							type="text"
							class="w-1/2"
							rows="4"
							@change="() => (flags.utilities = true)"
					...
					<ul class="grid grid-cols-3">
						<li
							v-for="(utility, index) in project.utilities.listOfUtilities"
							:key="utility"
					...
					<div class="container mt-3" style="float: left">
						...
						>
							...
							<div class="flex justify-evenly">
								<button
									name="edit-button"
									@click="showUpdateProjectImageModal(index)"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="17"
								...
								<button
									name="delete"
									@click="showDeleteProjectImageModal(index)"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="17"
				...
				<expand-panel title="Thông tin SEO">
					<div class="my-2">
						<button
							...
							@click="() => $modal.show('add-new-page-information')"
						>
							Thêm thông tin mới
						</button>
					</div>
					<div
						v-for="(item, index) in project.pageInfors"
						:key="index"
					...
					>
						<div class="flex justify-between items-center">
							...
							<div class="flex justify-around items-center space-x-1">
								<button
									name="edit-page-infor"
									@click="showPageInforModal(index)"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="17"
								...
								<button
									...
									name="delete"
									@click="
										sendWarningNotification('Không được xóa thông tin này!')
									"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
								...
								<button
									v-else
									name="delete"
									@click="showDeletePageInforModal(index)"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="17"
				...
				<expand-panel title="Nội dung SEO thuê">
					<div>
						...
						<button
							class="text-white px-3 py-1 bg-green-400 rounded relative float-right mt-3.5"
							@click="updateProjectInformation"
						>
							Cập nhật
						</button>
		...
		<modal name="page-information-detail">
			...
			<div class="p-2 m-2">
				<p class="mb-3">
					...
					<input
						v-model="project.pageInfors[pageInforIndex].title"
						type="text"
						class="w-1/2"
						style="width: 50% !important"
						@change="() => (flags.pageInfor = true)"
					/>
				</p>
				...
				<p class="mb-3">
					...
					<input
						v-model="project.pageInfors[pageInforIndex].slug"
						type="text"
						class="w-1/2"
						style="width: 50% !important"
						@change="() => (flags.pageInfor = true)"
				...
				<p>
					...
					<textarea
						v-model="project.pageInfors[pageInforIndex].metaDescription"
						type="text"
						class="w-1/2"
						rows="4"
						@change="() => (flags.pageInfor = true)"
				...
				<div class="flex justify-end space-x-3 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('page-information-detail')"
					>
						Quay lại
					</button>
					<button
						class="px-3 py-1 bg-green-400 rounded"
						@click="updatePageInformation"
					>
						Đồng ý
					</button>
				</div>
		...
		<modal name="delete-page-information">
			...
			<div class="p-2 m-2">
				...
				<div class="flex justify-end space-x-3 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('delete-page-information')"
					>
						Quay lại
					</button>
					<button
						class="px-3 py-1 bg-green-400 rounded"
						@click="deletePageInfor"
					>
						Đồng ý
					</button>
				</div>
		...
		<modal name="add-new-page-information">
			...
			<div class="p-2 m-2">
				...
				<p>
					<label for="meta" class="font-semibold">Meta description:</label>
					<textarea
						v-model="newPageInfor.metaDescription"
						type="text"
						class="w-1/2"
						rows="4"
					/>
		...
		<modal name="google-map-edit-modal">
			<div class="pt-12">
				...
				<div class="flex justify-end space-x-3 m-2 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('google-map-edit-modal')"
					>
						Quay lại
					</button>
					<button
		...
		<modal name="delete-project-utilities">
			<div class="pt-14">
				...
				<div class="flex justify-end space-x-3 m-2 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('delete-project-utilities')"
					>
						Quay lại
					</button>
					<button
						class="text-white px-3 py-1 bg-green-400 rounded"
						@click="deleteProjectUtilities(projectId)"
					>
						Đồng ý
					</button>
				</div>
			...
			<div class="pt-12">
				...
				<p class="m-10">
					...
					<input
						...
						@change="() => (flags.image = true)"
					/>
				</p>
				<div class="flex justify-end space-x-3 m-2 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('edit-project-image')"
					>
						Quay lại
					</button>
					<button
		...
		<modal name="show-edit-investor">
			<div v-if="investorEdit" class="p-5">
				...
				<div class="flex justify-between" style="height: 170px">
					<div>
						<div class="flex justify-between mb-2">
							...
							<input
								v-model="investorEdit.investorName"
								style="width: 50%"
								type="text"
								@change="() => (flags.investor = true)"
							/>
						</div>
						<div class="flex justify-between mb-2">
							<p class="font-bold">Số dự án</p>
							<input
								v-model="investorEdit.numberOfProjects"
								style="width: 50%"
								type="number"
								@change="() => (flags.investor = true)"
							/>
						...
						<div class="flex justify-between mb-2">
							...
							<input
								v-model="investorEdit.phoneNumber"
								style="width: 50%"
								type="text"
								max="10"
								@change="() => (flags.investor = true)"
				...
				<div class="flex justify-end space-x-3 m-2 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('show-edit-investor')"
					>
						Quay lại
					</button>
					<button
		...
		<modal name="edit-project-master-plan">
			<div class="pt-12">
				...
				<div class="flex justify-end space-x-3 m-2 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('edit-project-master-plan')"
					>
						Quay lại
					</button>
					<button
		...
		<modal name="delete-project-image">
			<div class="pt-14">
				...
				<div class="flex justify-end space-x-3 m-2 my-2">
					<button
						class="px-3 py-1 bg-gray-300 rounded"
						@click="$modal.hide('delete-project-image')"
					>
						Quay lại
					</button>
					<button
...
const getProject = gql`
	query GetProjectToEdit($condition: ProjectCollectionFilterInput) {
...
const getInvestorById = gql`
	query GetInvestorToEdit($input: InvestorCollectionFilterInput) {
...
export default {
	...
	apollo: {
		...
		investorEdit: {
			...
			skip() {
				return !this.isShowingEditInvestor;
			},
	...
	watch: {
		...
		inputFilter(filter) {
			...
			if (filter.project) {
				...
				setTimeout(() => {
					this.project.pageInfors.forEach(x =>
						this.pageInforsSlug.push(x.slug),
					);
				}, 1000);
	...
	methods: {
		...
		sendUpdateProjectMutation(queryable) {
			this.$apollo.mutate({
				mutation: gql`
					mutation UpdateProjectInformation($input: UpdateProjectInput!) {
		...
		addNewUtility() {
			if (
				this.project.utilities.listOfUtilities.some(
					x => x.toLowerCase() == this.tempUtility.toLowerCase(),
				)
			) {
			...
			this.$apollo.mutate({
				mutation: gql`
					mutation AddNewUtility($input: UpdateProjectInput!) {
						updateProject(input: $input) {
							string
						}
					}
		...
		addNewTempInfor() {
			if (
				this.project.tempProjectInfors.some(
					x => x.inforName.toLowerCase() === this.tempInfor.toLowerCase(),
				)
			) {
				this.sendWarningNotification("Đã tồn tại tiện ích này!");
			...
			this.project.tempProjectInfors.push({
				inforName: this.tempInfor,
				inforContent: "",
			});
			this.sendDoneNotification("Thêm thành công!");
		...
		updateProjectImage() {
			...
			if (this.project.images[this.imageIndex] === "") {
				this.sendWarningNotification(
					"Dữ liệu không được để trống, hoặc bạn có thể xóa đi hình ảnh này!",
				);
				return;
			}
			this.sendUpdateProjectMutation({
		...
		deleteProjectImage() {
			this.project.images = this.project.images.filter(
				x => x !== this.project.images[this.imageIndex],
			);
			this.$apollo.mutate({
				mutation: gql`
					mutation DeleteProjectImage($input: UpdateProjectInput!) {
						updateProject(input: $input) {
							string
						}
					}
		...
		updateProjectDetailInformation() {
			...
			if (Math.floor(this.project.numberOfApartments) < 0) {
				this.sendWarningNotification(
					"Số lượng căn hộ phải lớn hơn hoặc bằng 0!",
				);
				return;
			}
			if (Math.floor(this.project.numberOfBuildings) < 0) {
				this.sendWarningNotification(
					"Số lượng tòa nhà phải lớn hơn hoặc bằng 0!",
				);
				return;
			}
			...
			this.project.tempProjectInfors.forEach(x => {
		...
		ShowModalEditInvestor() {
			...
			if (foundedTime.getDate() < 10 && foundedTime.getMonth() + 1 > 10) {
				this.tempFoundedDate =
					foundedTime.getFullYear() +
					"-" +
					(foundedTime.getMonth() + 1).toString() +
					"-" +
					"0" +
					foundedTime.getDate();
			} else if (
				foundedTime.getDate() < 10 &&
				foundedTime.getMonth() + 1 < 10
			) {
				this.tempFoundedDate =
					foundedTime.getFullYear() +
					"-0" +
		...
		updateInvestor() {
			...
			this.investorEdit.numberOfProjects = Number(
				this.investorEdit.numberOfProjects,
			);
			...
			this.$apollo.mutate({
				...
				variables: {
					input: {
						...
						foundedTime:
							this.tempFoundedDate.substring(5, 7) +
							"-" +
							this.tempFoundedDate.substring(8, 10) +
							"-" +
							this.tempFoundedDate.substring(0, 4),
					},
				},
			});
			if (
				this.investors.some(
					x => x.investorName == this.investorEdit.investorName,
				) == false
			) {
				setTimeout(() => {
		...
		addNewPageInfor() {
			if (
				this.newPageInfor.slug === "" ||
				this.project.pageInfors.some(x => x.slug == this.newPageInfor.slug)
			) {
				this.sendWarningNotification(
					"Slug đã tồn tại hoặc chưa có, vui lòng thử lại!",
				);
				return;
			}
			const pageInfor = {
